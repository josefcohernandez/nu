...
**Resolución** (aplicada en [agente.md](../contracts/agente.md) §10): la extensión `agent` **acuña formalmente `EAGENT`** como su código de error estructurado (forma de api.md §1.4, mismo patrón con que providers.md §3 acuña `EPROVIDER`): errores propios del motor —`agent.toml` mal formado, `max_turns` agotado, subagente roto— viajan como `{ code = "EAGENT", message, detail? }`. La implementación ya lo lanzaba (`eagent()` en `agent/init.lua`, `subagent_worker.lua`); `chat.md` §8 y ADR-017, que ya lo citaban, quedan correctos sin tocarse. El defecto era la acuñación ausente en el contrato normativo.
