...
> **▶ Próxima sesión: `M13`** (loader, plugins y extensiones oficiales). Ya están
> probados TODOS los mecanismos del backend: marshaling (M05), scheduler por
> corrutinas (M06), cancelación (M07), eventos (M08), primitivas sync+⏸ (M09),
> handles (M10), UI (M11) y **workers = instancias wasm aisladas** (M12). M13 es la
> **integración con el Runtime**: `require` curado sobre wasm (DM5), carga
> topológica de las 8 extensiones embebidas, y **cablear el catálogo real** de
> primitivas (fs/proc/http/... contra las implementaciones Go del kernel) y el
> compositor real (compositor.go) a la interfaz `UIBackend`. Criterio de hecho:
> `nu --default-config && nu` funciona entero en wasm; la lista de skips queda
> VACÍA. Pendiente arrastrado: watchdog por época (DM4). · Bitácora abajo.
