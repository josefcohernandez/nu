...
> **▶ Próxima sesión: `M12`** (workers = instancias wasm). El **mecanismo** de UI
> está probado (M11): nu.ui.* como primitivas host contra la interfaz `UIBackend`,
> Region/Block como handles (sobre M10), y la pila de input + resolución de
> secuencias en el preludio (como el bus de eventos). El **compositor real**
> (compositor.go, VM-agnóstico según el censo) se enchufa a `UIBackend` en la
> integración con el Runtime (M13); ahí también arrancan de verdad la pantalla
> desnuda y el chat. M12 explota el **aislamiento físico de memoria** que da cada
> Instance (el regalo de ADR-019): un worker = una instancia wasm, con caps
> (deny-by-default, G6) y colas acotadas (G8). Pendiente: watchdog por época
> (DM4). · Bitácora abajo.
