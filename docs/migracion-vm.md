...
> **▶ Próxima sesión: `M10`** (userdata como handles). El **mecanismo** de
> primitivas (sync + ⏸) está probado (M09); el **registro del catálogo completo**
> (fs/proc/http/ws/search/text/codecs) contra las implementaciones Go del kernel
> es volumen mecánico que se hace en la **integración con el Runtime** (M13),
> donde esas implementaciones se envuelven como HostFn. Pendientes: watchdog por
> época (DM4), input de UI (M11). · Bitácora abajo.
