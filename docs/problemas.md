...
el 2026-07-13 con la opción (b), `RunTasks` persistente (su construcción es
la próxima sesión del plan: puntero ▶ de
[implementacion.md](implementacion.md)); G45–G46 —la superficie [W] de los
...
**Resolución** (2026-07-13; opción (b) — **`RunTasks` persistente**. El diseño queda decidido aquí y reflejado en [modelo-ejecucion.md](modelo-ejecucion.md) §limitaciones; la **construcción es la próxima sesión del plan** —el puntero ▶ de [implementacion.md](implementacion.md) la describe—, porque `scheduler.go`/`driver.go` son zona 🔒 y esto no es un parche). Tres piezas:
...
El modo interactivo lanza ese `RunTasks` de larga vida junto al driver — cierra la manifestación (1)—; `inst.mu` sigue siendo el **único token de entrada a la VM**, y la disciplina que el bucle residente necesita ya existe (`schedStep` toma y suelta el candado por paso, `scheduler.go:22`; las esperas ocurren sin él): no se introduce mecanismo de concurrencia nuevo, solo un usuario más del existente — el riesgo residual es de *vivacidad*, no de corrupción, y lo cubren los tests de la sesión. El modo headless conserva su semántica como condición de salida del mismo bucle (retornar en quiescencia de primer plano). **No toca `api.md`** (APILevel intacto): el contrato de `nu.task.every` pasa a cumplirse tal como se lee. Tests 🔒 obligatorios nombrando G44: un `every` sigue latiendo tras la quiescencia; una task encolada desde `EmitEvent`/`FeedInput` corre en tiempo acotado; el turno del agente completa sobre el driver de TTY (el camino del chat oficial); el input responde durante un paso largo (el watchdog acota el slice); apagado limpio sin goroutines fugadas. Al construir, tener [P33](pospuesto.md) a la vista (ctx en `HostFn`): no se resuelve aquí, pero el rediseño del despacho no debe cerrarle la puerta (su disparador ya cita este G##).
