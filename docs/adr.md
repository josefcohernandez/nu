...
- **Pendiente del dueño del proyecto, fuera de este ADR:** no hay `LICENSE` en el
  repo (los `tar.gz` llevan solo el binario hasta que se decida una); un flag
  `--version` en el CLI sería un nice-to-have de producto (toca la superficie CLI
  de S45), no un requisito de esta tubería; firmar binarios (cosign/GPG), brew
  tap y Docker quedan como mejoras futuras que reabrirían el punto 5.
