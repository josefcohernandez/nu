...
1. **El puntero** (esta misma línea, la única imperativa del documento):
   ...
   > **▶ Próxima sesión: `S06`** · Fase 1 · S05 cerrada (`task.sleep`/`defer`/`every` + `Timer:stop`). El scheduler sigue realizándose **sin yields de corrutina** (goroutine-por-task + token Lua, **ADR-011**, grieta **G31**). S06 (`nu.task.future`) y S07–S09 se construyen sobre ese modelo; S06 está en el inventario 🔒 (set una sola vez → `EINVAL`; varios `await` ven el valor ya resuelto).
