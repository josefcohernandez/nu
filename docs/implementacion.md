...
1. **El puntero** (esta misma línea, la única imperativa del documento):
   ...
   > **▶ Próxima sesión: `S09`** · Fase 1 · S08 cerrada (cancelación PÚBLICA: `Task:cancel()` + `nu.task.cleanup` con pila LIFO + `ECANCELED` observable en `await`; **desenrollado NO capturable por `pcall`/`xpcall`** formalizado envolviendo esos globales para que re-lancen el centinela `abortSignal` cuando la task está abortando —flag `task.aborting`—, atravesando cualquier `pcall` de usuario hasta `runTask`, que corre los `cleanup` en LIFO y descarta el desenlace). El scheduler sigue realizándose **sin yields de corrutina** (goroutine-por-task + token Lua, **ADR-011**, grieta **G31**). S09 (watchdog) depende de S08 (cerrada) y **reutiliza el mismo desenrollado**: el watchdog cortará el slice excedido lanzando el mismo `abortSignal` con `reason = abortBudget` (en vez de `abortCancel`), que `await` observará como `EBUDGET` y que emitirá `core:plugin.misbehaved` (verificable tras S10). Está en el inventario 🔒.
