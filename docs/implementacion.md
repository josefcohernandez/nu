...
1. **El puntero** (esta misma línea, la única imperativa del documento):
   ...
   > **▶ Próxima sesión: `S08`** · Fase 1 · S07 cerrada (`nu.task.all` alineado con inputs (G27) + `nu.task.race`, índice 1-based; ambos sobre un **substrato de cancelación interno** mínimo —`cancelCh`/`cancelTask`/pánico `abortSignal`— que aborta a las perdedoras en su siguiente ⏸, sin superficie pública nueva). El scheduler sigue realizándose **sin yields de corrutina** (goroutine-por-task + token Lua, **ADR-011**, grieta **G31**). S08 (cancelación PÚBLICA) depende de S04 (cerrada) y **reutiliza/extiende** ese substrato: expondrá `Task:cancel()`, `nu.task.cleanup` (pila LIFO) y `ECANCELED` observable en `await`, y debe **formalizar el desenrollado no capturable por `pcall`** —el centinela `abortSignal` ya existe, pero S07 lo dejó capturable por un `pcall` que envuelva el ⏸ (basta para `all`/`race`); S08 lo blinda. Está en el inventario 🔒.
