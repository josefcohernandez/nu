...
//   - `enu.fs.list` (S14) — recorrido recursivo del árbol, escrito en Lua.
//   - `enu.fs.read` (S14) — lectura del contenido de cada fichero.
//   - `enu.proc.run` (S16) — `git status --porcelain` con `opts.cwd`, sin shell.
//   - `enu.json.encode` (S18) — el resumen final, con `pretty`.
//   - `enu.json.decode` (S18) — re-parseo del resumen para validarlo.
...
func TestCP4HerramientaSoloConPrimitivas(t *testing.T) {
	...
	h.eval(`
		...
		enu.task.spawn(function()
			...
			-- Recorrido recursivo del árbol con enu.fs.list (sustituye a search.files,
			...
			local function recorrer(dir, rel)
				for _, e in ipairs(enu.fs.list(dir)) do
					if e.name ~= ".git" then
	...
	// enu.json.decode (cierra el círculo codec ida y vuelta) y validado en Go.
	h.expectEval(`
		local r = enu.json.decode(resumen_json)
