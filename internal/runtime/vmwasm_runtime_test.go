...
// binario `enu -e`. Cubre el catálogo síncrono (version/sys/codecs), una primitiva ⏸
// real (enu.fs por disco), red real (enu.http contra httptest) y la concurrencia del
// scheduler wasm (enu.task).
//
...
// así que `enu.ui` no se registra; el resto del catálogo (síncrono + ⏸) sí. Verifica
...
// M13d.2: enu.sys.platform() por el Runtime → un string no vacío (catálogo síncrono
// real: la primitiva corre en Go y cruza la frontera).
func TestRuntimeWasmSysPlatform(t *testing.T) {
	rt := newWasmRuntime(t)
	if got := evalStringOne(t, rt, `return enu.sys.platform()`); got == "" {
		t.Fatal("enu.sys.platform() devolvió vacío")
	}
}

// M13d.3: round-trip de un códec por EvalString (enu.json.encode∘decode). Prueba que
...
func TestRuntimeWasmJSONRoundTrip(t *testing.T) {
	...
	got := evalStringOne(t, rt, `
		local s = enu.json.encode({ nombre = "nu", n = 3, tags = { "a", "b" } })
...
// disco con enu.fs.write/enu.fs.read. Prueba de punta a punta el scheduler wasm + una
...
func TestRuntimeWasmTaskFsRoundTrip(t *testing.T) {
	...
	got := evalTaskOne(t, rt, `
		enu.fs.write("`+path+`", "contenido real de disco")
		return enu.fs.read("`+path+`")`)
	if got != "contenido real de disco" {
		t.Fatalf("fs round-trip: got %q", got)
	}
}

// M13d.5: red REAL por el Runtime — enu.http.request contra un httptest.NewServer.
...
func TestRuntimeWasmTaskHTTPRequest(t *testing.T) {
	...
	t.Cleanup(srv.Close)
	got := evalTaskOne(t, rt, `
		local resp = enu.http.request({ url = "`+srv.URL+`" })
...
// M13d.6: concurrencia por el Runtime — dos tasks con enu.task.sleep distinto se
// intercalan por el scheduler wasm (la de 5 ms termina antes que la de 20 ms). Prueba
// que EvalTaskString corre como task, que puede lanzar sub-tasks, y que enu.task.all
...
func TestRuntimeWasmTaskConcurrencia(t *testing.T) {
	...
	got := evalTaskOne(t, rt, `
		local traza = {}
		enu.task.all({
			function() enu.task.sleep(20); traza[#traza+1] = "a" end,
			function() enu.task.sleep(5);  traza[#traza+1] = "b" end,
		})
...
// (se lee la tabla del error en Lua). enu.fs.read de un fichero inexistente → ENOENT.
func TestRuntimeWasmTaskStructuredError(t *testing.T) {
	rt := newWasmRuntime(t)
	noexiste := filepath.ToSlash(filepath.Join(t.TempDir(), "noexiste"))
	_, err := rt.EvalTaskString(`return enu.fs.read("` + noexiste + `")`)
	if err == nil {
...
func TestRuntimeWasmWatchdogEBUDGET(t *testing.T) {
	...
	go func() {
		out, err := rt.EvalTaskString(`
			local w = enu.task.spawn(function() while true do end end)
			return enu.task.await(w)`)
		ch <- res{out, err}
	}()
	select {
	case r := <-ch:
