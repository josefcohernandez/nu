...
// Tests de los codecs `enu.json`/`enu.toml`/`enu.yaml` (api.md §12, sesión S18, 🔒).
...
// `enu.task.spawn`; uno de los snippets los ejercita además desde dentro de una
...
func TestJSONUTF8Estricto(t *testing.T) {
	...
	se := h.evalErr(`return enu.json.encode(string.char(0xff))`)
	...
	se = h.evalErr(`return enu.json.encode({ a = "ok", b = string.char(0x80, 0x80) })`)
	if se.Code != CodeEINVAL {
	...
	se = h.evalErr(`return enu.json.encode({ [string.char(0xfe)] = 1 })`)
	if se.Code != CodeEINVAL {
	...
	h.expectEval(`
		local s = "café 漢字 🚀"
		local enc = enu.json.encode(s)
		return tostring(enu.json.decode(enc) == s)
	`, "true")
}

// TestJSONSentinelNull blinda la otra mitad del 🔒 de S18: el **sentinel NULL
// ida y vuelta**. `decode('{"a":null,"b":1}')` deja la clave `a` como
// `enu.json.NULL` (PRESENTE, no perdida) y `b == 1`; `encode` de una tabla con
// `a = enu.json.NULL` produce `"a":null`. El round-trip no pierde claves —que es
...
func TestJSONSentinelNull(t *testing.T) {
	...
	h.expectEval(`
		local t = enu.json.decode('{"a":null,"b":1}')
		assert(t.a == enu.json.NULL, "a debe ser el sentinel NULL, no nil")
	...
	h.expectEval(`
		return enu.json.encode({ a = nil, b = 1 })
	`, `{"b":1}`)
}
...
// (`{}`), la decisión documentada (docs/worklog/README.md S18).
func TestJSONArrayVsObjeto(t *testing.T) {
	h := newHarness(t)

	// Secuencia 1..n → array.
	h.expectEval(`return enu.json.encode({10, 20, 30})`, `[10,20,30]`)

	// Mapa con claves string → objeto (orden de claves estable: alfabético).
	h.expectEval(`return enu.json.encode({nombre = "ana", edad = 30})`, `{"edad":30,"nombre":"ana"}`)

	// Tabla vacía → objeto {} (decisión documentada).
	h.expectEval(`return enu.json.encode({})`, `{}`)

	// Mezcla (1 contiguo + clave string) → objeto, no array (no es secuencia pura).
	h.expectEval(`return enu.json.encode({ [1] = "x", clave = "y" })`, `{"1":"x","clave":"y"}`)

	// decode de un array → secuencia Lua 1..n.
	h.expectEval(`
		local a = enu.json.decode('[7,8,9]')
		assert(#a == 3 and a[1] == 7 and a[3] == 9, "array decodificado")
		return "ok"
	`, "ok")

	// Anidamiento: array de objetos.
	h.expectEval(`return enu.json.encode({ {n=1}, {n=2} })`, `[{"n":1},{"n":2}]`)
}
...
func TestJSONPretty(t *testing.T) {
	...
	got := h.eval(`return enu.json.encode({ a = 1, b = {2, 3} }, { pretty = true })`)
	pretty := got[0]
	...
	// canónico.
	h.expectEval(`
		local p = enu.json.encode({ a = 1, b = {2, 3} }, { pretty = true })
		return enu.json.encode(enu.json.decode(p))
	`, `{"a":1,"b":[2,3]}`)
}
...
func TestJSONDecodeInvalido(t *testing.T) {
	h := newHarness(t)
	se := h.evalErr(`return enu.json.decode('{ roto: ')`)
	if se.Code != CodeEINVAL {
...
requires = ["base", "ui"]
		...
		local t = enu.toml.decode(manifiesto)
	...
	se := h.evalErr(`return enu.toml.decode("clave = ")`)
	if se.Code != CodeEINVAL {
	...
	se = h.evalErr(`return enu.toml.encode({1, 2, 3})`)
	if se.Code != CodeEINVAL {
		t.Fatalf("toml.encode de raíz no-objeto debe lanzar EINVAL, llegó %q", se.Code)
	}
}

// TestTOMLDecodeArrayDeTablas blinda que `enu.toml.decode` convierte un
...
enabled: true
	...
	h.expectEval(`
		...
		local back = enu.yaml.decode(enu.yaml.encode(orig))
	...
	se := h.evalErr(`return enu.yaml.decode("{ a: 1, b")`)
	if se.Code != CodeEINVAL {
...
func TestCodecsDesdeTask(t *testing.T) {
	...
	h.eval(`
		...
		enu.task.spawn(function()
			local j = enu.json.encode({ a = 1 })
			assert(j == '{"a":1}', "json en task")
			local tm = enu.toml.decode('k = "v"')
			assert(tm.k == "v", "toml en task")
			local y = enu.yaml.decode("x: 1")
...
func TestEncodeNoSerializable(t *testing.T) {
	...
	se := h.evalErr(`return enu.json.encode(function() end)`)
	if se.Code != CodeEINVAL {
		t.Fatalf("encode de función debe lanzar EINVAL, llegó %q", se.Code)
	}

	se = h.evalErr(`return enu.json.encode(0/0)`) // NaN
	if se.Code != CodeEINVAL {
		t.Fatalf("encode de NaN debe lanzar EINVAL, llegó %q", se.Code)
	}

	se = h.evalErr(`return enu.json.encode(1/0)`) // +Inf
	if se.Code != CodeEINVAL {
