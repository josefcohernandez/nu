...
// runScript evalúa `setup` (que crea tasks con enu.task.spawn) y conduce el bucle
...
func TestSchedConcurrencia(t *testing.T) {
	out := runScript(t, `
		...
		out = ""
		enu.task.spawn(function()
			enu.task.sleep(20); traza[#traza+1] = "A1"
			enu.task.sleep(20); traza[#traza+1] = "A2"
		end)
		enu.task.spawn(function()
			enu.task.sleep(5);  traza[#traza+1] = "B1"
			enu.task.sleep(5);  traza[#traza+1] = "B2"
		end)
...
func TestSchedAwaitYaTerminada(t *testing.T) {
	out := runScript(t, `
		out = "no"
		enu.task.spawn(function()
			local w = enu.task.spawn(function() return "listo" end)
			enu.task.sleep(10)  -- da tiempo a que w termine
			out = enu.task.await(w)
		end)`)
	if out != "listo" {
		t.Fatalf("got %q", out)
	}
...
func TestSchedContextCancel(t *testing.T) {
	...
	if _, lerr, err := inst.Eval(`
		enu.task.spawn(function() enu.task.sleep(60000) end)`); err != nil || lerr != "" {
